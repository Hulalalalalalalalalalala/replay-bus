...
- `Bus.replay(from = 0) -> Promise<Array<{ seq, id, record }>>` — inclusive of `from`, ascending. The messages come from the lock-free read view, so a replay never queues behind (or blocks) a writer; the replay counter it advances is a write and still goes through the mutex.
...
- `Bus.read(name) -> Array<{ seq, id, record }>` — messages after the consumer's position, same shape as replay. Does not move the position; empty array past the end. Synchronous and lock-free; an unknown name is auto-registered first.
- `Bus.readRange(start, limit) -> Array<{ seq, id, record }>` — paged message fetch. Takes only a start sequence number and a maximum count; returns the same shape as `replay`/`read` and changes no sequence, position or statistic. Messages are returned ascending starting **at** `start` (inclusive), at most `limit`, with fewer returned toward the tail; a start past the end gives `[]`. A start inside a truncated gap begins at the earliest surviving message, and a fully deleted range gives `[]` (never an error). `start` must be a non-negative integer and `limit` a positive integer — either violation throws `TypeError` synchronously. It is served entirely from the lock-free read view, stays available after `close` (like `stats`/`usage`), and repeated fetches have no side effects.
...
- `Bus.truncate(before) -> Promise<void>` — retention truncation at segment granularity. `before` must be a non-negative integer (anything else throws `TypeError` synchronously); after `close` the call rejects with `Error`. Every truncate seals the active segment and rolls a fresh one; then, oldest first, each whole segment whose newest message is `<= before` is deleted — but only once every registered consumer's position has passed that segment. Segments a consumer still needs stay put (no error), a segment is never half-deleted, a bound of `0` or nothing eligible is a no-op, and a bound past the end behaves as the end. Replay and read of surviving ranges are unchanged (seq, record, order); a start inside a deleted range begins at the earliest surviving message (empty array when nothing survives), and a consumer positioned below the earliest surviving seq reads from that first survivor — the truncated gap is skipped, never repeated. Dedup keys keep their first acknowledgement across truncation and restarts, and `stats()` never shrink. Truncation runs on the same serial chain as publish/compact: invocation order decides. Reads (`read`, `readRange`, and the data side of `replay`) do not take the chain.
...
The merged state is kept between drains by an incremental reader of the active log tail (keyed by device/inode and the log's first-line fingerprint), so the write path never does the full directory walk; compaction and truncation (which replace the head) transparently fall back to one full scan. The drain also holds a short, bounded coalescing window while more than one live session shares the lease: requests staged while it waited for the mutex (or a heartbeat after its first gather) are re-gathered into the **same** group, so genuinely concurrent writers across processes land in one bracketed commit rather than taking a second lock round. The epoch credential is re-validated at every pipeline boundary (lock entry, decision, each flush chunk, commit, settlement): a holder taken over while queued or mid group fails its whole request, and its group either never commits or is filtered wholesale by the epoch fence — no stale byte and no stale staging file mixes into the log. A write or fsync failure rolls the entire group back physically, leaving the bus byte-for-byte as it was. A crash leaves either the complete group or none of it: the uncommitted bracket/torn tail is severed and the decision reconciles the survivors on the next open.

## Read view

Reads are decoupled from the write chain entirely. `read`, `readRange`, the data side of `replay`, position lookups, `stats` and `usage` are served from a read-only view that never takes the write mutex and never walks the directory on the hot path:

- a steady-state refresh stats the active log and parses only the bytes appended since the previous look, so fetching a range costs in proportion to the batch returned and **does not grow with log length**;
- a structural change (a segment seal, a truncation, or a compaction replacing the log head) is detected from the device/inode pair and the log's first-line fingerprint and swaps in one coherent rebuild, so a read racing a snapshot switch observes the whole old view or the whole new one — the same range fetched either side agrees message for message;
- reads always observe a **committed prefix**: a commit group is either wholly visible or wholly invisible (an in-flight bracket is skipped), sequences are continuous with no repeats, and the cumulative stats and positions only move forward;
- message payloads are located, not cached — each committed message is an offset/length locator and the record is read back from its segment or the active log, so the view's memory stays bounded by the message *counts*, not their bodies (snapshot-folded records remain parsed).

Reads therefore keep working while writers commit continuously, and a displaced (fenced) instance keeps reading as before: only its writes and truncations are invalidated. `readRange`, `stats` and `usage` also remain available after `close`, serving the last committed cache. Only two read-adjacent acts still take the mutex: the replay counter line written by `replay` and the auto-registration of an unknown consumer by `read`/`register`/`advance`.

## Tests

    npm test

The suite covers the single-process behaviour (unchanged) and cross-process scenarios driven through child processes: interleaved publishes/batches from two live writers, fencing of a displaced writer, shared dedup across processes and restarts, positions advanced from two processes, merged quota rejection, monotone merged stats, concurrent truncation/compaction, and recovery from stale leases, torn lines and orphaned snapshots. It also covers the lock-free read view: `readRange` paging through truncation and compaction (including across restart), reads answered while another process holds the drain, consistency across repeated snapshot switches, reads after close and after a takeover, and the drain coalescing window merging genuinely concurrent writers into one commit group.

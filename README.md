...
- `Bus.usage() -> number` — business bytes currently retained by messages that have not been truncated away (zero on an empty bus). Unchanged by compaction; drops as `truncate` discards segments. Readable after `close`. A publish (or whole batch) whose effective records would push `usage()` past `maxBytes` is rejected with `RangeError` and leaves every state — seq, stats, dedup, bytes — untouched; landing exactly on the limit succeeds, and resending an existing `dedupKey` always succeeds by reusing its first acknowledgement without consuming quota. The check runs on the same serial chain as publish/replay/compact/truncate, so invocation order decides it.
